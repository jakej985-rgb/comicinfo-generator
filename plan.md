...
The implementation currently had a fallback similar to:
...
- AND metadata valid
- AND merge succeeds
- AND archive validation succeeds

Anything else becomes:

`REVIEW` or `FAILED` depending on the failure.
...
- `NOT_FOUND` from `PROVIDER_FAILURE` from `PROVIDER_OFFLINE`
...
`MANUAL_REVIEW` unless there is explicit authoritative evidence resolving the difference.
...
The current candidate comparison uses series, issue number, and approximate year. That is not enough for all comic numbering.
...
Current archive verification checks CRC and file size. That is good for normal operation, but a strict verification mode should provide stronger guarantees.
...
```text
original entry SHA256 == temporary entry SHA256
```
...
- Use **CRC + size** for fast verification.
- Use **SHA256** for strict verification.
...
Do not hash every archive entry multiple times during normal operation unless configured. The Phase 53 benchmarks should determine the performance impact.
...
The archive writer currently treats some filesystem durability failures as non-fatal. That is inappropriate for safety-critical durability operations.
...
For production mode, the safest behavior is `archive update = FAILED` rather than pretending the operation is durable.
...
```text
write temp → fsync temp → replace → fsync final → fsync directory
```
...
This is the final gate. Unit tests passing is not enough. Do not immediately point the application at the entire library.
...
Only after dry-run is correct, process a small batch (e.g. 5 files).
...
```text
Kapowarr metadata → resolver → ComicInfo
```
...
Watcher sees `comic.cbz` and processes it.

The resulting write must not trigger endless processing (expected: 1 processing cycle).

---

### 63.2 Restart test

Process file, restart application, and verify it does not immediately reprocess the same archive.

---

### 63.3 Rapid modification test

Simulate: file appears → file changes → file changes again.
...
Force a provider/archive failure and verify `FAILED/REVIEW` does not produce an infinite retry loop.
...
Run multiple workers simultaneously (e.g., worker-1, worker-2, worker-3, worker-4 against 100 jobs).

Verify each job is processed once unless intentionally retried.
...
```text
lease expires → job reclaimed → new worker processes it
```
...
Create a permanently failing archive and verify:

```text
attempt 1 → attempt 2 → attempt 3 → FAILED (no infinite loop)
```
...
- `except Exception:\n    pass`
- `int(issue_number)`
- direct provider imports from `api/`
- archive operations outside the archive writer
- `os.replace`, `shutil.move`, `os.remove`, `open(..., "w")`
...
```text
56 Identity / Metadata separation
        ↓
57 Provider failure propagation
        ↓
58 Identity authority/conflict rules
        ↓
59 Canonical identity comparison
        ↓
60 Strict archive verification
        ↓
61 fsync durability handling
        ↓
62 Real-library integration
        ↓
63 Automation stress testing
        ↓
64 Multi-worker stress testing
        ↓
65 Final safety audit
        ↓
66 Documentation
        ↓
67 Final regression
