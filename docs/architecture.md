...
services/
  ...
  story_arc.py         — Story arc search, issue retrieval, and library fix service
   ↓
pipeline/
  resolver.py          — MetadataResolver: two-phase identity → metadata + ProviderOperationResult
  ...
  conflicts.py         — formal conflict detection (XML, provider, variant, volume, year)
...
models/
  ...
  identity.py          — ComicIdentity, CanonicalIdentityKey, IdentityEvidence dataclasses
   ↓
providers/
  kapowarr/            — Kapowarr REST API (with library snapshot caching)
  comicvine/           — Comic Vine HTML scraper & API
  gcd/                 — Grand Comics Database scraper
  story_arc.py         — story arc search + reading-order parser
   ↓
writers/
  archive.py           — 10-step atomic CBZ embed (CRC verify → strict SHA256 → fsync file → fsync dir → os.replace → post-verify)
...
cache/
  ...
  jobs.py              — JobStore (Durable queue, WAL mode, BEGIN IMMEDIATE leases, max_attempts recovery)
...
automation/
  ...
  queue.py             — ProcessingQueue (ThreadPoolExecutor with active job deduplication)
...
| `pipeline/resolver.py` | Two-phase identity resolution & metadata retrieval with provider operation result tracking |
| `pipeline/confidence.py` | Candidate pool evaluation, score-margin protection, and multi-field agreement bonus |
| `pipeline/dry_run.py` | In-memory SQLite context and archive write interception for true dry-run isolation |
| `writers/archive.py` | 10-step atomic archive writes with entry-by-entry CRC/SHA256 verification and explicit fsync classification |
| `cache/jobs.py` | Durable SQLite queue in WAL mode with worker lease locking and poison-pill limit recovery |
...
2. **No archive modification without entry-level verification** — `embed_comicinfo_in_cbz` always runs `verify_cbz_archive` before and after `os.replace`.
...
7. **Strict Separation of Identity and Metadata** — no direct transition from Identity Resolution to Archive Write.
